...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Nunito','Segoe UI',sans-serif",background:"linear-gradient(165deg,#e8f4f8 0%,#d1ecf1 30%,#e2f0e8 60%,#f0f7f4 100%)",minHeight:"100vh",color:"#2c4a52"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1a6b7a 0%,#2d8f6f 50%,#1a6b7a 100%)",padding:"20px 20px 16px",color:"#fff",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-40,right:-40,width:160,height:160,borderRadius:"50%",background:"rgba(255,255,255,0.06)"}}/>
        <div style={{maxWidth:920,margin:"0 auto",position:"relative"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:26}}>💚</span>
            <h1 style={{margin:0,fontSize:26,fontFamily:"'Playfair Display',serif",fontWeight:800}}>Health 401(k)</h1>
          </div>
          <p style={{margin:"2px 0 0 38px",opacity:.75,fontSize:13}}>VO2 max · assisted care avoidance · weight-adjusted risk · cascade prevention</p>
        </div>
      </div>

      <div style={{maxWidth:920,margin:"0 auto",padding:"0 14px 36px"}}>

        {/* Connected Input Panel */}
        <div style={{background:"#fff",borderRadius:14,padding:"16px 18px",marginTop:12,boxShadow:"0 2px 16px rgba(26,107,122,.08)"}}>
        ...
        {/* Tabs */}
        <div style={{display:"flex",gap:2,marginTop:12,background:"#fff",borderRadius:10,padding:3,boxShadow:"0 1px 6px rgba(0,0,0,.04)"}}>
        ...
        {calc && <div style={{marginTop:12}}>

          {/* DASHBOARD */}
          {tab==="dashboard" && <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(135px,1fr))",gap:8,marginBottom:12}}>
              <div style={{position:"relative",animation:savingsHint?"hintPulse 1.5s ease-in-out infinite":"none",borderRadius:12}} onClick={function(){if(savingsHint)setSavingsHint(false)}}>
                <SC l="Health Net Worth" v={fmt(calc.total)} s={(endAge-startAge)+"yr"+(showInv?" compounded":"")} c="#1a6b7a" i="💰"/>
                {savingsHint && <div style={{position:"absolute",top:-8,right:-8,left:-8,bottom:-8,borderRadius:14,border:"3px solid #1a6b7a",animation:"hintPulse 1.5s ease-in-out infinite",pointerEvents:"none"}}/>}
              </div>
              <SC l="Weight" v={calc.finalWt+" lbs"} s={"-"+calc.wtLost+" lbs (BMI "+calc.endBMI+")"} c="#2d8f6f" i="⚖️"/>
              <SC l={"VO2 Max ("+endAge+")"} v={""+calc.finalVO2} s="ml/kg/min" c="#e85a3a" i="🫀"/>
              <SC l="Care Avoided" v={fmt(calc.cumCare)} s="vs sedentary" c="#8a5ac4" i="🏥"/>
              <SC l="Wellness" v={calc.ful+"/100"} s={calc.weeklyMETmin>=1000?"Elite":"Active"} c="#4a9a8a" i="✨"/>
            </div>

            {savingsHint && calc.actBk && calc.actBk.length > 0 && <div onClick={function(){setSavingsHint(false)}} style={{
              background:"linear-gradient(135deg,#1a6b7a,#2d8f6f)",borderRadius:14,padding:"20px 24px",marginBottom:12,
              cursor:"pointer",animation:"fadeIn 0.5s ease-out",position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:-20,right:-20,width:120,height:120,borderRadius:"50%",background:"rgba(255,255,255,0.06)"}}/>
              <div style={{display:"flex",alignItems:"center",gap:16,position:"relative"}}>
                <div style={{fontSize:44}}>🚶</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,color:"rgba(255,255,255,.7)",fontWeight:600,marginBottom:4}}>
                    Every 30-min walk saves you
                  </div>
                  <div style={{fontSize:32,fontWeight:800,color:"#fff",letterSpacing:"-1px",marginBottom:6}}>
                    {"$"+calc.actBk[0].perSession.toFixed(0)} saved
                  </div>
                  <div style={{fontSize:12,color:"rgba(255,255,255,.8)",lineHeight:1.5}}>
                    Walk 3 times a week and save <b style={{color:"#fff"}}>{fmt(calc.total)}</b> by age {endAge}
                  </div>
